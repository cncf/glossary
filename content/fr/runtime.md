...
Dans le contexte des [applications Cloud Natives](/fr/cloud-native-apps/), l'environnement d'exécution fait généralement référence à l'environnement d'exécution des conteneurs (ou container runtime en anglais).
Un environnement d'exécution des conteneurs implémente la spécification [Open Container Initiative](https://opencontainers.org/) pour assurer une gestion cohérente des différentes technologies d'orchestration de conteneurs.

## Problème auquel il répond

Sans l'abstraction d'un environnement d'exécution, l'application devrait gérer toute la mécanique de chaque système d'exploitation, augmentant la complexité de l'exécution de l'application.

## Quel en est l'utilité

L'environnement d'exécution des conteneurs est un composant nécessaire des orchestrateurs de conteneurs tels que Kubernetes.
Il gère le cycle de vie des conteneurs, et fait principalement trois choses.
Premièrement, il définit comment les images de conteneurs sont spécifiées et comment l'environnement d'exécution peut les récupérer.
...
Avec le temps, différents produits d'environnement d'exécution des conteneurs ont évolué, conduisant à la spécification OCI,
qui est devenue la norme pour les environnements d'exécution des conteneurs.
