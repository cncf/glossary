...
영어로 된 확실한 용어 정의를 바탕으로, 경험이 적은 기여자가 용어를 대상 언어로 현지화할 수 있다. 
...
검토 과정을 용이하게 하기 위해 [의미를 고려한 줄 바꿈](https://sembr.org/)(예: 한 문장당 한 줄씩)을 사용한다.
GitHub에서 Markdown 텍스트 형식(예: 하이퍼링크, 굵은 글씨, 이탤릭체)을 올바르게 지정할 수 있도록 
...
문서 서식 지정 및 작성에 대한 가이드라인을 이해하고 기여 프로세스를 보다 효율적으로 만들기 위해 [스타일 가이드](/ko/style-guide/)를 읽는다.
...
관심있는 용어가 이미 다른 사람에게 할당되지는 않았는지 확인한다. 예를 들어, 처음 세 용어는 아직 할당되지 않았지만 네 번째 용어는 이미 할당되었음을 알 수 있다.
...
_@iamnoah_, _@nate-double-u_, _@Seokho Son_, _@Jihoon Seo_, 및/또는 _@castrojo_ 를 태그하여 이를 확인하도록 한다.

다음 단계는 [신규 용어 제안(PR 생성)](#submitting-a-new-term) 섹션을 참조한다.

**참고**: 메인터이너가 이슈를 할당한 후 이슈에 대한 작업을 시작할 수 있다.
한 번에 한 용어에 대한 작업만 신청할 수 있다.
여러 용어에 대한 작업은 순차적으로 진행되므로, 다음 용어에 대한 작업을 신청하기 전에 이전 용어에 대한 작업을 완료해야 한다.
...
새로운 용어 제안을 메인테이너들이 놓치지 않도록 Slack에서 _@iamnoah_, _@nate-double-u_, _@Seokho Son_, _@Jihoon Seo_, 및/또는 _@castrojo_ 를 태그하여 알린다. 
해당 용어 정의에 대해 작업하고 싶은 경우, 메인테이너에게 알리면 해당 용어를 할당해줄 것이다.

### 새로운 용어 추가하기(PR 열기) {#submitting-a-new-term-creating-a-pr}

[스타일 가이드](/ko/style-guide/)에 설명된 바와 같이 Google 또는 Word 문서에서부터 시작하는 것을 추천한다.
...
[스타일 가이드](/ko/style-guide/)를 읽고 가이드라인을 준수했는지 확인한다.
